...
export function findTheBat(contacts:Contact[]): string|null {
    let batman = contacts.find(element => element.name=="Batman");
    if(batman){
        return batman.address
    }else{
        return null;
    }
}
...
export function generateNewUser(name:string): User {
    const user = {name, verified:false};
    return user;
}   
 ...
 */
export function getUser(id:number,object:Object): User|null {
